...
const calc = (size, material, options, promocode, result) => {
  ...
  const calcFunc = () => {
    ...
    if (sizeBlock.value == '' || materialBlock.value == '') {
      resultBlock.textContent = "Пожалуйса, выберите размер и материал картины";
...
const forms = () => {
  ...
  form.forEach(item => {
    item.addEventListener('submit', e => {
      ...
      textMessage.textContent = message.lodading;
...
const scrolling = upSelector => {
  ...
  //                 smoothScrooll(scrollTop, hashElementTop, this.hash);
  //          }
  //      });
  // } 
  // const smoothScrooll = (from, to, hash) => {
  //     let timeInterval = 1,
  //         prevScrollTop, 
  //         speed;
...
const showMoreStyles = (trigger, wrapper) => {
  ...
  btn.addEventListener('click', function () {
    (0,_services_requests__WEBPACK_IMPORTED_MODULE_0__.getData)(' http://localhost:3000/styles').then(res => createCard(res)).catch(error => console.log(error));
    this.remove();
  });
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  (0,_modules_scrolling__WEBPACK_IMPORTED_MODULE_11__["default"])('[href^="#"]');
